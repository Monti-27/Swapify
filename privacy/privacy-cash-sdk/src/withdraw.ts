import { Connection, Keypair, LAMPORTS_PER_SOL, PublicKey, Transaction, TransactionInstruction, VersionedTransaction, TransactionMessage, SystemProgram, ComputeBudgetProgram } from '@solana/web3.js';
...
export async function withdraw({ recipient, lightWasm, storage, publicKey, connection, amount_in_lamports, encryptionService, keyBasePath, transactionSigner }: WithdrawParams) {
    ...
    // If transactionSigner is provided, build and return unsigned transaction
    if (transactionSigner) {
        // Build the actual Solana transaction
        ...
        // Call the transaction signer (which will capture the unsigned transaction)
        versionedTransaction = await transactionSigner(versionedTransaction);

        logger.debug('Transaction signed by user');
        return { isPartial, tx: 'unsigned_transaction_captured', recipient: recipient.toString(), amount_in_lamports, fee_in_lamports };
    }

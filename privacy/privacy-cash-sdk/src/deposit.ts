...
export async function depositUnsigned({ lightWasm, storage, keyBasePath, publicKey, connection, amount_in_lamports, encryptionService, transactionSigner, referrer }: DepositParams) {
    // check limit
    let limitAmount = await checkDepositLimit(connection)
    if (limitAmount && amount_in_lamports > limitAmount * LAMPORTS_PER_SOL) {
        throw new Error(`Don't deposit more than ${limitAmount} SOL`)
...
export async function relayDeposit(serializedTransaction: string, publicKey: PublicKey, encryptedOutput1: Uint8Array, referrer?: string): Promise<string> {
    try {
        // Relay the pre-signed transaction to indexer backend
        logger.info('submitting transaction to relayer...')
        const signature = await relayDepositToIndexer(serializedTransaction, publicKey, referrer);

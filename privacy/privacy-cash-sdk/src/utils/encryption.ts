 ...
 */
  ...
  /**
   * Generate an encryption key from a wallet signer (for use with Privy or other wallet services)
   * @param signer The wallet signer that can sign messages
   * @returns The generated encryption key
   */
  public async deriveEncryptionKeyFromSigner(signer: WalletSigner): Promise<EncryptionKey> {
    // Sign the same constant message that's used for keypair-based derivation
    const message = Buffer.from('Privacy Money account sign in');

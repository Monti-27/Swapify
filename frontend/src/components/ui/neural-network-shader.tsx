...
const fragmentShader = `
  ...
  void main() {
    ...
    vec4 signal = cppn_fn(uv, 0.1 * sin(0.3 * iTime), 0.1 * sin(0.69 * iTime), 0.1 * sin(0.44 * iTime));
    
    // Mix between the two theme colors based on the signal intensity
    // Use signal.x (red channel) as the mixing factor
    vec3 finalColor = mix(uColor1, uColor2, signal.x);
    
    // Force opaque alpha - preventing it from vanishing
...
// Add this type declaration for the shader component
...
function ShaderPlane({ color1, color2 }: { color1: string, color2: string }) {
    ...
    // Calculate dimensions to preserve the "zoomed" look and top-alignment
    // Original locked look was height ~4 vs viewport ~2.3 (approx 1.75x)
    // And centered at -0.75, which aligned the top of mesh with top of screen.
    ...
    return (
        <mesh
            ref={meshRef}
            scale={[viewport.width * widthScale, viewport.height * heightScale, 1]}
            position={[0, -viewport.height * 0.4, 0]}
        >
            <planeGeometry args={[1, 1]} />
            <neuralShaderMaterial ref={materialRef} side={THREE.DoubleSide} transparent={true} />
        </mesh>
    );
}
...
export function ShaderBackground() {
    ...
    // Defaut to dark mode colors if not mounted yet to avoid hydration mismatch
    ...
    return (
        <div ref={canvasRef} className="absolute inset-0 w-full h-full pointer-events-none" aria-hidden>
            ...
            {/* Overlay gradient adjusted for themes */}
            <div className={`pointer-events-none absolute inset-0 bg-gradient-to-t ${isDark ? 'from-black/80 via-transparent to-black/20' : 'from-white/80 via-transparent to-white/20'}`} />
        </div>
    );
}

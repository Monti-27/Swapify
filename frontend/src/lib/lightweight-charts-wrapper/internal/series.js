...
import { AreaSeries, BarSeries, CandlestickSeries, HistogramSeries, LineSeries, BaselineSeries } from 'lightweight-charts';
...
function createSeries(chart, params) {
    switch (params.type) {
        case 'Area': {
            const series = chart.addSeries(AreaSeries, omit(params));
            const defaults = clone(series.options());
            series.setData(params.data);
            return [series, defaults];
        }
        case 'Bar': {
            const series = chart.addSeries(BarSeries, omit(params));
            const defaults = clone(series.options());
            series.setData(params.data);
            return [series, defaults];
        }
        case 'Candlestick': {
            const series = chart.addSeries(CandlestickSeries, omit(params));
            const defaults = clone(series.options());
            series.setData(params.data);
            return [series, defaults];
        }
        case 'Histogram': {
            const series = chart.addSeries(HistogramSeries, omit(params));
            const defaults = clone(series.options());
            series.setData(params.data);
            return [series, defaults];
        }
        case 'Line': {
            const series = chart.addSeries(LineSeries, omit(params));
            const defaults = clone(series.options());
            series.setData(params.data);
            return [series, defaults];
        }
        case 'Baseline': {
            const series = chart.addSeries(BaselineSeries, omit(params));
            const defaults = clone(series.options());
            series.setData(params.data);
            return [series, defaults];
        }
        case 'Custom': {
            const series = chart.addSeries(params.view, omit(params));
            const defaults = clone(series.options());
            series.setData(params.data);
            return [series, defaults];
        }
